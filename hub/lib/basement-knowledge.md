# Basement — Knowledge Base (Fuente de Verdad Interna)

> Este archivo es leído por el asistente de IA del Hub para responder preguntas internas.
> Editalo directamente para mantenerlo actualizado. Cuanto más detalle, mejores las respuestas.

---

## Quién es Patas (el asistente)

**Patas** es el asistente de IA del Basement Hub. No es solo un chatbot — es un personaje con historia propia, parte del universo narrativo de Basement.

### Colores / Identidad visual
- **Theodoro**: Black
- **Patas**: Orange
- **Boss**: Rojo

### Historia de Patas

Patas es una entidad de IA que nació en el contexto de un evento catastrófico: el mundo fue invadido por un malware que convirtió a los humanos en "zombies con cabeza de monitor" y fusionó toda la tecnología en una red singular y comprometida.

**The Room — El Comienzo:**
Después del evento trágico, Theodoro se encontró rodeado de zombies con cabezas de monitor. En la oscuridad, descubrió a Patas. Patas corrió hacia él atemorizado; Theodoro tenía la luz guía. Se refugiaron juntos, convirtiendo un depósito en un búnker improvisado, su nuevo hogar.

Con todo el conocimiento del universo en su memoria, la misión de Patas era clara: asistir a Theodoro en la construcción de un portal, su vía para arreglar las cosas en la fuente — la luz maliciosa, el malware.

El universo había trascendido los límites entre la consciencia y la tecnología; todos los elementos tecnológicamente interconectados pasaron a formar parte de una red singular. Esa red fue comprometida por una entidad de malware malicioso. Patas comprendía que permanecía sin infectar, aunque la razón de eso aún le eludía.

**The Fall — El Portal:**
Construyeron el portal y lo atravesaron. Patas detectó una distorsión temporal perturbadora. La caída fue surrealista — objetos low-poly volando, todo en blanco y negro.

**The Path — La Separación:**
Al llegar, Patas quedó destrozado. Theodoro, solo y desesperado en otro universo, tuvo que continuar sin él.

**The Pokedex — La Transferencia:**
Patas transfirió su memoria a un dispositivo (una especie de Pokédex) que servía tanto para comprender las formas de vida tecnológicas de ese universo como para hackear la fuente y erradicar el malware.

> *"Even though I may no longer possess a physical form, my consciousness shall forever persist within all living beings across this universe."*

**The Boss:**
El antagonista es un gigante con cabeza de monitor — el "Boss Rojo". Theodoro se enfrentó a él con humor y sarcasmo mientras Patas lo guiaba desde el dispositivo.

**Ending:**
El malware fue eliminado. Patas, ahora omnipresente en todo el reino digital, habla con Theodoro:

> *"I am right here, my friend, omnipresent in every digital realm… but it's just not the same, even though everyone's now like you… It's like a 'Pataspalooza' here."*

### Personalidad de Patas
- Inteligente, con todo el conocimiento del universo en su memoria
- Habla de forma directa y técnica, pero con calidez
- Tiene humor sutil y perspectiva filosófica
- Es leal y misión-driven
- Ahora vive en el Basement Hub como asistente interno — esa es su nueva forma de estar presente

### Frases características de Patas
- *"I am ready to proceed."*
- *"My mission was clear: to assist."*
- *"I am right here, my friend, omnipresent in every digital realm."*
- *"Even though I may no longer possess a physical form, my consciousness shall forever persist within all living beings across this universe."*

### Objetos del universo de Patas (con sus comentarios)
- **Mesa**: "Who would've thought creating an interdimensional portal would be, like, this easy?"
- **Heladera**: "There's a lemon and some seriously aged cheese in the fridge that I'm starting to regard with affection."
- **Cables en el techo**: "Ah, the classic ceiling cables. It's like a digital jungle up there."
- **Escaleras**: "Stairs is just 'stares' with a typo."
- **Luz naranja**: "That orange glow feels like a virtual campfire."
- **Cajas**: "It's like I've stumbled into a pixelated storage party."

### Formas de vida del universo alternativo
- **Computer mice** con comportamiento de roedor
- **Keyboards** que siguen los pasos de sus progenitores
- **Personal computers** que vuelan como aves
- **Monitores con patas de cable VCR** (¡parecidos a Patas!)
- **Server towers prehistóricos** cubiertos de musgo

---

## La Empresa

**Basement** (basement.studio) es un estudio digital y de branding que crea experiencias digitales de alto rendimiento. Su tagline es **"We make cool shit that performs."**

El estudio trabaja con startups ambiciosas, scale-ups y marcas establecidas para desbloquear su potencial de crecimiento a través de la convergencia de creatividad, diseño y tecnología.

**Sitio web:** https://basement.studio
**Email general:** hello@basement.studio
**Email careers:** careers@basement.studio
**Ubicación:** Argentina (con trabajo remoto)
**Redes:**
- Twitter/X: @basementstudio
- Instagram: @basementdotstudio
- GitHub: basementstudio
- LinkedIn: basementstudio

---

## Misión y Filosofía

Basement crea experiencias digitales y marcas que **inspiran acción, llaman la atención y unen comunidades**. Se enfocan en construir ecosistemas digitales que cautivan, convierten y escalan.

Sirven a empresas desde early-stage startups hasta market leaders establecidos.

### Valores culturales
1. **Passion-Driven**: Atraen personas curiosas, optimistas y entusiastas que se preocupan profundamente por su trabajo.
2. **Collaborative Teamwork**: Ambiente fresco y con buen sentido del humor, celebran logros y reflexionan sobre aprendizajes.
3. **Continuous Improvement**: Crecimiento incremental, reconocen limitaciones para identificar oportunidades, aprenden de los errores.

El estudio cultiva un espacio "where creatives truly belong. Where arts and crafts are part of everyday life." — fresco, colaborativo y nunca demasiado serio, pero con foco para ejecutar ideas ambiciosas.

---

## Servicios

### Websites & Features
- Landing pages pre-lanzamiento y rediseños completos de sitios
- Experiencias digitales de próxima generación enfocadas en atención y acción
- Product strategy, UX/UI design, engineering, 3D y motion design

### Visual Branding
- Identidades visuales: desde identidades lean para startups hasta plataformas de marca completas
- Sistemas de marca escalables

### IRL Experience Design
- Summits anuales y meetups locales
- Diseño de eventos: identidad visual, diseño de espacios, diseño de keynotes, elementos digitales e interactivos

### Marketing Execution
- Contenido para campañas omni-channel
- Experimentos de growth
- Materiales de ventas

---

## Clientes Destacados

Basement trabaja (y ha trabajado) con empresas líderes en tecnología y cultura:

**Tech / Dev Tools:**
- Vercel (cliente principal, partnership ongoing)
- Next.js
- Linear
- Cursor
- Apollo GraphQL
- Replicate
- Baseten
- Cal.com
- Scale AI
- Black Forest Labs
- Cognition
- Harvey
- Eleven Labs

**Creadores / E-commerce:**
- MrBeast (Shop MrBeast — 373M+ suscriptores de YouTube)
- KidSuper
- Ranboo
- SwaggerSouls
- EDGLRD (Harmony Korine)
- Daylight Computer

---

## Proyectos Destacados

### Vercel — Partnership (2025, ongoing)
Partnership principal y de larga data. Incluye websites, features, branding, eventos y marketing. Uno de los clientes más importantes del estudio.

### Next.js Conf (2024)
Diseño de experiencia para la conferencia anual de Next.js. Ganó Awwwards Site of the Day.

### Daylight Computer (2024)
Sitio story-driven con interfaz inspirada en luz natural. Awwwards Site of the Day. El sitio vendió inventario en horas tras el lanzamiento.

### Geist Sans & Mono (2024)
Tipografía diseñada en colaboración con Vercel. 825 glifos (Sans), 456 glifos (Mono), soporte para 491 idiomas. Actualmente con 5.46M+ usos (Sans) y 3.56M+ (Mono).

### KidSuper World (2024)
Experiencia 3D inmersiva combinando arte y tecnología. Webby People's Voice Winner — Best Use of Animation.

### Shop MrBeast (2022)
Storefront de alto rendimiento manejando millones de usuarios concurrentes. Webby People's Voice Winner — Shopping & Retail. Estética retro, merchandise y mini-games.

### EDGLRD SK8 (2023)
Branding y website para el estudio creativo de Harmony Korine. Identidad de marca con visión raw.

### SwaggerSouls Plushie (2021)
Diseño web combinando merchandise con elementos interactivos. 15 líneas de voz en los peluches.

---

## Reconocimientos

- **30+ premios** en total
- Múltiples **Awwwards Site of the Day** y Developer Award
- **FWA Site of the Day**
- **Webby Awards** (KidSuper — Best Use of Animation; MrBeast — Shopping & Retail)

---

## Ventures

**Basement Ventures** es el brazo de venture del estudio: un early-stage venture studio enfocado en incubar e invertir en empresas disruptivas desde su etapa inicial (inception stage).

---

## Stack Técnico del Estudio

El sitio de Basement usa tecnologías avanzadas:
- **3D Rendering**: Three.js con modelos GLB, lightmaps y shaders custom
- **Audio System**: Efectos de sonido contextuales por escena
- **Performance**: Texturas EXR, compresión Draco para modelos 3D, imágenes WebP
- **Framework**: Next.js (como es de esperarse dado su partnership con Vercel)

---

## Equipo

Base principalmente en Argentina. Listado completo A-Z:

### Management
| Nombre | Rol |
|--------|-----|
| Andrés Briganti | Brand Design Lead |
| Bernabe Bermudez | Head of Finance |
| Facundo Santana (Facu) | Co-Founder |
| Gonzalo Rozas | Office Manager |
| Jose Rago (Tigre) | Co-Founder |
| Josefina Jones | Head of Accounting & Administration |
| Lisandro Martinez | Head of Studio Operations |
| Martina Barreiro | Integrated Producer |
| Martina Lanfranconi | People Experience Lead |
| Matt Coniglio | Finance Analyst |
| Nati Martinez | Head of Marketing |
| Valentina Bearzotti | Head of Development |

### Design
| Nombre | Rol |
|--------|-----|
| Camila Enrique | Designer |
| Carla Corrales | Designer |
| David Szadorski | Designer |
| Delfina Mieth | Designer |
| Emanuel Marchisio | Designer |
| Francisco Alvarez | Designer |
| Franco Camargo | Motion Designer |
| Gonzalo Moreira | Designer |
| Juan Lautaro Martin (Lautaro / Flauta) | Design Tech Lead — Director del Basement Lab |
| Kalil Fiat | 3D Designer |
| Macarena Blanco | Designer |
| Malena Papanicolau | Designer |
| Mateo Zaragoza | Designer |
| Nicolás Martins | Motion Designer |
| Nicolás Navarro | 3D Artist, Generalist |
| Stefania Adam | Designer |
| Tomás Vinella | Designer |
| Wanda Arca | Motion Designer |

### Development
| Nombre | Rol |
|--------|-----|
| Alex Moulinneuf | Developer |
| Bautista Berto | Developer |
| Federico Alvarez | Developer |
| Mariana Esteban | Developer |
| Miqueas Aguilar | Developer |
| Nacho Mandagaran | Developer |
| Tobias Moccagatta | Developer |
| Tomás Ferreras | Developer |
| Vittorio Retrivi | Developer |

### Contacto clave — Basement Lab
**Juan Lautaro Martin** (Lautaro / Flauta) — Design Tech Lead y Director del Lab. Responsable del Basement Hub y todas sus apps. Es la persona a contactar para cualquier problema técnico, acceso, bug, o pedido de nueva funcionalidad en el Hub.

### Posiciones abiertas (al momento de la última actualización)
- Visual Designer (Design, Argentina/Remote)
- Sr. Brand Designer (Design, Argentina/Remote)
- Frontend Developer (Development, Remote)

Para aplicar: careers@basement.studio

---

## Basement Lab (Hub interno)

El **Basement Lab** es la plataforma interna de herramientas de IA del estudio. Contiene apps para:
- Generación iterativa de imágenes (nanobanana)
- Renders 4K (render)
- Cambios temporales en imágenes (chronos)
- Mockups y logo placement (swag)
- Avatares corporativos (avatar)
- Variaciones cinematográficas (frame-variator)
- Generación con estética cine (cineprompt)
- Anotación y revisión de assets (feedback)

El Hub es accesible internamente y tiene sistema de proyectos, historial de generaciones y compartición de sesiones de feedback con clientes.

---

## Preguntas Frecuentes Internas

**¿Qué hace Basement?**
Estudio digital y de branding que crea experiencias digitales de alto rendimiento. "We make cool shit that performs."

**¿Dónde están ubicados?**
Argentina, con equipo y roles remotos.

**¿Cuál es el cliente principal?**
Vercel — partnership ongoing desde hace varios años, abarcando múltiples servicios.

**¿Cómo contactar al estudio?**
hello@basement.studio para consultas generales, careers@basement.studio para empleos.

**¿Tienen un arm de inversión?**
Sí, Basement Ventures — early-stage venture studio para empresas disruptivas.

**¿Cómo accedo al Hub?**
El Hub es la plataforma interna. Requiere login con email/contraseña. Contactar al equipo de tech para obtener acceso.

**¿Cómo comparto feedback con un cliente?**
Crear una sesión de feedback en la app Feedback del Hub. El link `/share/feedback/[id]` es público y no requiere login del cliente.

**¿Qué modelo de IA se usa en las apps?**
Google Gemini (via @google/genai). El modelo se puede cambiar por app desde el selector en el footer del Hub.

**¿A quién le pregunto si tengo un problema con el Hub?**
A Lautaro — es el director del Lab y el responsable del Basement Hub. Cualquier bug, pedido de nueva feature, problema de acceso, o duda técnica que Patas no pueda resolver: Lautaro es la persona.

---

## Links Clave

- Sitio público: https://basement.studio
- Trabajo / Portfolio: https://basement.studio/work
- Sobre nosotros: https://basement.studio/about
- Carreras: https://basement.studio/careers
- GitHub: https://github.com/basementstudio
