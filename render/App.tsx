  ...
  const handleDownload = async (url: string, id: string) => {
    if (isHubEnv()) {
      try {
        ...
        await openDownloadAction(dataUrl, 'render');
      } catch {
  ...
  return (
    <div className="flex h-screen bg-[#0a0a0a] overflow-hidden select-none">
      <Sidebar
        ...
        onReferenceClick={isHubEnv() ? () => openReferencePicker().then((dataUrl) => { setReferenceBase64(dataUrl); setReferenceUrl(dataUrl); }).catch(() => {}) : undefined}
        ...
        <div className="flex-1 overflow-y-auto p-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 pb-20">
          <div className="relative aspect-video border border-dashed border-[#333] flex items-center justify-center hover:border-zinc-500 transition-colors group cursor-pointer overflow-hidden">
            ...
            <div className="text-center font-mono">
              <div className="text-2xl text-zinc-600 group-hover:text-zinc-400 mb-2">+</div>
              <div className="text-[10px] text-zinc-600 uppercase tracking-widest group-hover:text-zinc-400">Import Viewport Previews</div>
            </div>
          </div>
              ...
              <div className="flex-1 relative bg-[#111]">
                ...
                {job.status === 'rendering' && (
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className="text-[10px] text-zinc-400 uppercase tracking-widest animate-pulse font-mono">
                      Computing 4K...
                    </div>
                  </div>
                )}
                ...
                {job.status === 'completed' && job.resultUrl && (
                   <button 
                    onClick={() => handleDownload(job.resultUrl!, job.id)}
              ...
              <div className="h-10 border-t border-[#333] flex items-center justify-between px-3 text-[9px] uppercase tracking-tighter font-mono">
                <div className="flex items-center space-x-3">
                  ...
                  <span className={
                    ...
                    job.status === 'failed' ? 'text-red-500' : 'text-zinc-600'
                  }>
                    {job.status}
                  </span>
                </div>
